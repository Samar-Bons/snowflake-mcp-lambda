...
def mock_services() -> tuple[Mock, Mock]:
    ...
    mock_gemini_service.translate_nl_to_sql.return_value = "SELECT id, name FROM customers LIMIT 2;"

    return mock_schema_service, mock_gemini_service


@pytest.fixture
def authenticated_client(client: TestClient, mock_user: User, mock_services: tuple[Mock, Mock]) -> Generator[TestClient, None, None]:
...
class TestChatEndpoints:
    ...
    def test_chat_endpoint_success_with_autorun(
        ...
        mock_schema_service.execute_query.assert_called_once_with("SELECT id, name FROM customers LIMIT 2;")
    ...
    def test_chat_endpoint_success_without_autorun(
        ...
        mock_gemini_service.translate_nl_to_sql.return_value = "SELECT * FROM customers WHERE age > 25;"

        # Make request
        request_data = {
            "prompt": "Show me customers older than 25",
            "autorun": False
        }
    ...
    def test_chat_endpoint_gemini_error(
        ...
        mock_gemini_service.translate_nl_to_sql.side_effect = GeminiServiceError("API error")
    ...
    def test_chat_endpoint_schema_discovery_error(
        ...
        mock_schema_service.discover_schema.side_effect = SchemaServiceError("Connection failed")
    ...
    def test_chat_endpoint_query_execution_error(
        ...
        mock_schema_service.execute_query.side_effect = SchemaServiceError("Query failed")
        mock_gemini_service.translate_nl_to_sql.return_value = "SELECT * FROM customers;"
    ...
    def test_execute_sql_endpoint_success(
        ...
        # Make request
        request_data = {
            "sql": "SELECT id, name, email FROM customers LIMIT 2;"
        }
    ...
    def test_execute_sql_endpoint_invalid_sql(
        ...
        mock_schema_service.execute_query.side_effect = SchemaServiceError("Only SELECT queries are allowed")
